...
use App\Http\Requests\API\StateAPIRequest;
...
class CityAPIController extends AppBaseController
{
     ...
     * By State.
     */
    public function byState(StateAPIRequest $request)
    {
        $input = $request->all();
        $result = City::where('state_id', $input['state_id'])->get();
        return $this->sendResponse($result->toArray(), 'By State');
    }
